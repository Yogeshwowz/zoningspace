...
var pageURL = $(location).attr("href");
	...
	// Create table list
			function createTable(result,sno,pagno){
				var radio_search_hidden = $('#radio_search_hidden').val();
				if(radio_search_hidden==1){
					$class ="hidecls";
				}
				if(radio_search_hidden==2){
					$class ="";
				}
				...
				if(countTable > 0){
					for(index in result){
						...
						if ($.inArray(sid, arr)!='-1') {
							...
							tr+='<th class="thcheckboxcls '+$class+'"><input class="checkboxesrec" type="checkbox" name="checkrecords[]" value="'+sid+'"  checked '+$checked+'></th>';
						}else{
							tr+='<th class="thcheckboxcls '+$class+'"><input class="checkboxesrec" type="checkbox" name="checkrecords[]" value="'+sid+'"  '+$checked+'></th>';
						}
...
/********* Document End here *********/
   ...
   $(".login_popup_search").click(function() { 
	...
	$("#member-register-frm input[required]").keyup(function() { 
	 ...
	 $('body').on('click', '#add_to_cart_subscription', function() {
		...
		if(radio_search_hidden==1){
			if(parseInt(allcount) > parseInt(connects)){
					...
					swal({
							 title: "Are you sure you want to buy "+allcount+" search records",
							  text: "",
							  icon: "warning",
							  buttons: [
								'No, cancel it!',
								...
								var formData = $("#search-form").serialize();
										...
										$.ajax({
											...
											data: {
												formdata : formData,
												allcount: allcount,
												radio_search_type: radio_search_hidden,
												allcount: allcount
											...
											success: function(msgData) {
											 H5_loading.hide();
											 if(msgData==1){
												var connectsLeft =  parseInt(connects) - parseInt(allcount);
												$('#connects').val(connectsLeft);
												$('#connect_span').text(connectsLeft);
												$('#loading').show(); 
												$.ajax({
												type: "POST",
												url: baseUrl+'dashboard/downloadSearchRecord',
												data: {
													formdata : formData,
													allcount: allcount,
													radio_search_type: radio_search_hidden
		...
		if(radio_search_hidden==2){
				if(parseInt(buyrecordCount) > parseInt(connects)){
					 $('#add-to-cart-popup-subscription').modal('show');
				 }else{
					swal({
							 title: "Are you sure you want to buy "+buyrecordCount+" search records",
							  text: "",
							  icon: "warning",
							  buttons: [
								'No, cancel it!',
						  ...
						  if (isConfirm) {
							...
							var formData = $("#search-form").serialize();
								...
								$.ajax({
									...
									data: {
										formdata : formData,
										checkrecordIds : checkrecords,
										radio_search_type: radio_search_hidden,
										allcount: buyrecordCount,
										checkboxarray :arr
									...
									success: function(msgData) {
									 H5_loading.hide();
									 if(msgData==1){
										 var connectsLeft =  parseInt(connects) - parseInt(buyrecordCount);
										 $('#connects').val(connectsLeft);
										 $('#connect_span').text(connectsLeft);
										$('#loading').show(); 
										$.ajax({
	 ...
	 $('body').on('click', '.radio_search', function() {
		 if ($(".radio_search").is(':checked')) {
			var radio_search = $(this).val();
			if(radio_search==1){
				arr.splice(0,arr.length);
				$('.checkboxesrec,.CheckAll').prop("checked", false);
				$('#thcheckbox').addClass('hidecls');
				$('.thcheckboxcls').addClass('hidecls');
				$('#radio_search_hidden').val(radio_search);
			}
			if(radio_search==2){
				$('#thcheckbox').removeClass('hidecls');
				$('.thcheckboxcls').removeClass('hidecls');
				$('#radio_search_hidden').val(radio_search);
			}
		 }else{
