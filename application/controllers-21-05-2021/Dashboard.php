...
class Dashboard extends CI_Controller
{
	...
	// User Dash Board
		public function dashboard(){
			...
			$data['saverecords'] = $this->Dashboard_model->getSaveSearch();
		...
		public function purchasedDataList($id){
			$data['buyrecords'] = $this->Dashboard_model->purchasedDataList($id);
			$data['title'] = 'Buy Search';
			$this->load->view('templates/header');
			$this->load->view('user-dashboard/purchased-data-list', $data);
			$this->load->view('templates/footer');
		}
		...
		$data['records'] =$this->Dashboard_model->viewPurchasedDataSingleRecord($id);	
		$this->load->view('templates/header');
		$this->load->view('user-dashboard/view-purchased-record',$data);
		$this->load->view('templates/footer');
	}
		...
		public function downloadBuySearchRecord(){
			 $result = $this->Dashboard_model->downloadBuySearchRecord();
				$filename = date("d-m-Y").'_'.$this->session->userdata('user_id').'_'.rand().' properties.csv';
					$csvname = $_SERVER['DOCUMENT_ROOT'].'/zoningspace/assets/csv/'.$filename;
					$csvnamezip = date("d-m-Y").'_'.$this->session->userdata('user_id').'_'.rand().' properties.zip';
					$fileopen=fopen($csvname,'w');
		...
		public function manageSearchRecords(){
			$result = $this->Dashboard_model->manageSearchRecords();
			echo"<pre>";
			print_r($result);
	...
	function deleteMember(){
			$data = $this->Dashboard_model->deleteMember();
			echo $data;
	...
	public function buyConnect(){
	  $data['title'] = 'Buy Connect';
	  $this->load->view('templates/header');
	  $this->load->view('user-dashboard/buy-connect', $data);
	  $this->load->view('templates/footer');
	}	
}
