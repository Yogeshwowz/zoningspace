<?php
	...
	public function getrecordByID($id) {
	 ...
	 function getresearher(){
		$this->db->from('tbl_users');
        $this->db->where('roleId',3);
        $query = $this->db->get();
        return $query->result_array();
    }
	 function order_id_array($id){
		$order_id_array =implode(',',$id);
		 $Sql ="select tbl_save_search.*,tbl_orders.* from  tbl_orders
		 left join tbl_save_search on tbl_save_search.search_id=tbl_orders.search_id where tbl_orders.order_id IN (".$order_id_array.")";
		 $query = $this->db->query($Sql);
